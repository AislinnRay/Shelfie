import React from "react"
//import { withRouter } from "react-router-dom"
import styleHeader from './styleHeader.css'

function Header(props) {
  return (
    <header className="Header">
        <img className="logo" src="data:image/png;base64,"/>
        <p className="Shelfie">Shelfie</p>
        {/* <button className="header-button" onClick={() => props.history.push("/")}>Dashboard</button>
      <button className="header-button" id="add" onClick={() => props.history.push("/form")}>Add Inventory</button> */}
    </header>
  )
}

// export default withRouter(Header)
export default Header